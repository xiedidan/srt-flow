<script setup>
/**
 * Video Synthesis Tasks View
 * Enhanced with list/card view toggle, task details, and video preview
 */
...
import SynthesisTaskCreateModal from '@/components/synthesis/SynthesisTaskCreateModal.vue'
import Pagination from '@/components/common/Pagination.vue'

const tasksStore = useTasksStore()
const videosStore = useVideosStore()

const taskType = 'synthesize'
const { viewMode } = useViewMode('synthesis', 'card')
...
const videoPreviewTask = ref(null)
...
// Subtitle mode display names
const subtitleModeNames = {
  original_only: '仅原始',
  translated_only: '仅翻译',
  dual: '双字幕'
}
...
function handlePlayVideo(task) {
  if (!task.result?.output_path) return
  const videoId = task.payload?.video_id || task.video_id
  if (!videoId) return
  videoPreviewUrl.value = `/api/v1/videos/${videoId}/stream`
  videoPreviewTask.value = task
}

function closeVideoPreview() {
  videoPreviewUrl.value = null
  videoPreviewTask.value = null
}
...
function getVideoTitle(task) {
  if (task.video_title) return task.video_title
...
function getSubtitleMode(task) {
  const mode = task.payload?.subtitle_mode
  return subtitleModeNames[mode] || mode || '-'
}

function getCodec(task) {
  const codec = task.payload?.video_codec || task.result?.video_codec
...
<template>
  <div class="synthesis-tasks-view">
    <div class="view-header">
      <div class="header-content">
        <div class="title-row">
          <h1 class="view-title">🎞️ 视频合成</h1>
          <span class="ws-status" :class="{ connected }">
            {{ connected ? '● 实时' : '○ 离线' }}
          </span>
        </div>
        <p class="view-description">将字幕烧录到视频中生成最终输出</p>
      </div>
      ...
      <div v-if="viewMode === 'card' && paginatedTasks.length > 0" class="tasks-grid">
        ...
        >
          ...
          <div class="card-body">
            ...
            <div class="task-meta">
              <span class="mode-badge">{{ getSubtitleMode(task) }}</span>
              <span class="codec-badge">{{ getCodec(task) }}</span>
              <span v-if="task.result?.file_size" class="file-size">
          ...
          <div class="card-footer">
            ...
            <button 
              v-if="task.status === 'completed' && task.result?.output_path" 
              class="action-btn primary" 
              @click="handlePlayVideo(task)"
              title="预览视频"
            >
              🎬 预览
            </button>
        ...
        <table class="task-table">
          ...
          <tbody>
            <tr v-for="task in paginatedTasks" :key="task.id">
              ...
              <td class="mode-cell">{{ getSubtitleMode(task) }}</td>
              <td class="codec-cell">{{ getCodec(task) }}</td>
              ...
              <td class="size-cell">{{ formatFileSize(task.result?.file_size) }}</td>
    ...
    <!-- Video Preview Modal -->
    <Teleport to="body">
      <div v-if="videoPreviewUrl" class="video-preview-overlay" @click.self="closeVideoPreview">
        <div class="video-preview-modal">
          ...
          <div class="video-preview-body">
            <div class="video-info">
              <div class="video-title-preview">{{ getVideoTitle(videoPreviewTask) }}</div>
              <div class="video-meta-preview">
                <span>字幕: {{ getSubtitleMode(videoPreviewTask) }}</span>
                <span>编码: {{ getCodec(videoPreviewTask) }}</span>
                <span v-if="videoPreviewTask?.result?.file_size">
                  大小: {{ formatFileSize(videoPreviewTask.result.file_size) }}
                </span>
              </div>
            </div>
            <video 
