<script setup>
/**
 * Asset Generation Tasks View
 * Enhanced with list/card view toggle, status filter, and result display
 */
...
import AssetTaskCreateModal from '@/components/asset/AssetTaskCreateModal.vue'
import AssetResultView from '@/components/asset/AssetResultView.vue'
import Pagination from '@/components/common/Pagination.vue'

const tasksStore = useTasksStore()
const videosStore = useVideosStore()

const taskType = 'asset_gen'
const { viewMode } = useViewMode('asset', 'card')
...
function getGenerationItems(task) {
  const items = []
  if (task.payload?.generate_title) items.push('标题')
  if (task.payload?.generate_summary) items.push('摘要')
  if (task.payload?.generate_thumbnail) items.push('缩略图')
  return items.join('、') || '-'
}

function getGenerationItemBadges(task) {
  const badges = []
  if (task.payload?.generate_title) badges.push({ label: '📝', title: '标题' })
  if (task.payload?.generate_summary) badges.push({ label: '📄', title: '摘要' })
  if (task.payload?.generate_thumbnail) badges.push({ label: '🖼️', title: '缩略图' })
  return badges
}

function getTargetPlatform(task) {
  const platform = task.payload?.title_config?.target_platform
  const platformNames = {
    general: '通用',
    youtube: 'YouTube',
    bilibili: 'Bilibili',
...
<template>
  <div class="asset-tasks-view">
    <div class="view-header">
      <div class="header-content">
        <div class="title-row">
          <h1 class="view-title">🎨 素材生成</h1>
          <span class="ws-status" :class="{ connected }">
            {{ connected ? '● 实时' : '○ 离线' }}
          </span>
        </div>
        <p class="view-description">生成视频封面、标题、摘要等素材</p>
      </div>
      ...
      <div v-if="viewMode === 'card' && paginatedTasks.length > 0" class="tasks-grid">
        ...
        >
          ...
          <div class="card-body">
            ...
            <div class="task-meta">
              <span 
                v-for="badge in getGenerationItemBadges(task)" 
                :key="badge.title"
                class="gen-badge"
                :title="badge.title"
              >{{ badge.label }}</span>
              <span v-if="task.payload?.title_config?.target_platform" class="platform-badge">
                {{ getTargetPlatform(task) }}
              </span>
            </div>
        ...
        <table class="task-table">
          ...
          <tbody>
            <tr v-for="task in paginatedTasks" :key="task.id">
              ...
              <td class="items-cell">{{ getGenerationItems(task) }}</td>
              <td class="platform-cell">{{ getTargetPlatform(task) }}</td>
    ...
    <!-- Result View Modal -->
    <Teleport to="body">
      <div v-if="selectedResultTask" class="result-modal-overlay" @click.self="closeResultView">
        <div class="result-modal-container">
          <AssetResultView
            :task="selectedResultTask"
            :visible="!!selectedResultTask"
