<script setup>
/**
 * TTS (Text-to-Speech) Tasks View
 * Enhanced with list/card view toggle, task details, and audio preview
 */
...
import TtsTaskCreateModal from '@/components/tts/TtsTaskCreateModal.vue'
...
const taskType = 'tts'
const { viewMode } = useViewMode('tts', 'card')
...
const audioPreviewTask = ref(null)
...
function handlePlayAudio(task) {
  ...
  // Build audio URL from video_id
  const videoId = task.payload?.video_id || task.video_id
  if (!videoId) return
  
  audioPreviewUrl.value = `/api/v1/videos/${videoId}/audio/tts`
  audioPreviewTask.value = task
}
...
function getEngineName(task) {
  const engine = task.payload?.engine || task.result?.engine_used
...
<template>
  <div class="tts-tasks-view">
    <div class="view-header">
      <div class="header-content">
        <div class="title-row">
          <h1 class="view-title">🔊 语音合成</h1>
          <span class="ws-status" :class="{ connected }">
            {{ connected ? '● 实时' : '○ 离线' }}
          </span>
        </div>
        <p class="view-description">将翻译字幕转换为语音音频</p>
      </div>
      ...
      <div v-if="viewMode === 'card' && paginatedTasks.length > 0" class="tasks-grid">
        ...
        >
          ...
          <div class="card-body">
            ...
            <div class="task-meta">
              <span class="engine-badge">{{ getEngineName(task) }}</span>
              <span v-if="task.result?.duration" class="duration">
        ...
        <table class="task-table">
          ...
          <tbody>
            <tr v-for="task in paginatedTasks" :key="task.id" :class="statusConfig[task.status]?.class">
              ...
              <td class="engine-cell">{{ getEngineName(task) }}</td>
              ...
              <td class="duration-cell">{{ formatDuration(task.result?.duration) }}</td>
    ...
    <!-- Audio Preview Modal -->
    <Teleport to="body">
      <div v-if="audioPreviewUrl" class="audio-preview-overlay" @click.self="closeAudioPreview">
        <div class="audio-preview-modal">
          <div class="audio-preview-header">
            <h4>🎧 音频试听</h4>
            <button class="close-btn" @click="closeAudioPreview">✕</button>
          </div>
          <div class="audio-preview-body">
            <div class="audio-info">
              <div class="audio-title">{{ getVideoTitle(audioPreviewTask) }}</div>
              <div class="audio-meta">
                <span>引擎: {{ getEngineName(audioPreviewTask) }}</span>
                <span v-if="audioPreviewTask?.result?.duration">
                  时长: {{ formatDuration(audioPreviewTask.result.duration) }}
                </span>
              </div>
            </div>
            <audio 
